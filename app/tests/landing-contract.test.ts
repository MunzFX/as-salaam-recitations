...
describe("scroll-scrub website landing contract", () => {
  ...
  test("keeps public landing and full app routes separate", () => {
    ...
    const journey = readFileSync(new URL("../src/components/archive/journey.tsx", import.meta.url), "utf8");
    expect(landingRoute).toMatch(/component:\s*Journey/);
    expect(journey).toContain("<ScrollScrub");
