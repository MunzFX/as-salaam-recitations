...
export interface ScrollScrubScene {
  ...
  tags?: string[];
  actions?: ReactNode;
  /** Live, accessible product UI above the cinematic environment. */
  content?: ReactNode;
  align?: "left" | "right";
...
export function ScrollScrub({
  ...
  useEffect(() => {
    ...
    const smallViewport = window.matchMedia("(max-width: 600px)");
    const isMobile = () =>
      smallViewport.matches || (coarsePointer && window.innerHeight > window.innerWidth);
    ...
    const layout = () => {
      ...
      // The sticky stage consumes the final viewport. Finish the last film
      // before the stage leaves the screen, rather than holding it half-played.
      const last = runtime.at(-1);
      if (last) {
        last.end = Math.max(last.start + 1, last.end - viewportHeight);
      }
      total = Math.max(last?.end ?? viewportHeight, 1);
      dirty = true;
    };
    ...
    const readScroll = () => {
      ...
      for (const [index, segment] of runtime.entries()) {
        ...
        if (
          y > segment.start - 2.5 * viewportHeight &&
    ...
    const updateVideos = (elapsed: number) => {
      const blend = 1 - Math.exp(-elapsed / 85);
      for (const segment of runtime) {
        const { video } = segment;
        // Advance the playhead independently of decoder latency. Waiting for
        // each seek before easing again compounds lag during fast scrolling.
        if (!segment.visible) {
          segment.current = segment.target;
          continue;
        }
        segment.current += (segment.target - segment.current) * blend;
    ...
    const tick = (now: number) => {
      ...
      updateVideos(Math.min(64, Math.max(0, now - lastTick)));
      lastTick = now;
              ...
              >
                <picture className="scroll-scrub__picture">
                  {segment.mobilePoster ? (
                    <source
                      media="(orientation: portrait) and (pointer: coarse), (max-width: 600px)"
